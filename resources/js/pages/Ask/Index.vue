...
const form = useForm({
  ...
  model: props.selectedModel ?? (props.models.length ? props.models[0].id : ''),
})
...
<template>
  <div class="max-w-3xl p-8 mx-auto">
    ...
    <form @submit.prevent="submitForm">
      <!-- Choix du modèle -->
      <div class="mb-4">
        <label class="block mb-1 font-semibold">Modèle</label>
        <select v-model="form.model" class="w-full p-2 border rounded" required>
    ...
    <div v-if="response" class="mt-8">
      ...
      <div class="p-4 whitespace-pre-wrap bg-gray-100 rounded">
        {{ response }}
      </div>
    </div>
  </div>
...
button { padding: 10px 20px; cursor: pointer; margin-bottom: 10px; }
